...
/*   Updated: 2020/10/02 12:50:17 by hcabel           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
					...
					t_vector dir, float max_distance)
{
	...
	while (loop_index < RAY_LOOP && depth < max_distance)
	{
		...
		if (nearest_surface <= RAY_PRECIS)
		{
			...
			hitinfos.location = vectoradd(start_location,
				vectormult(dir, depth));
			return (hitinfos);
		}
	...
	hitinfos.distance = depth;
	hitinfos.recursion = loop_index;
	hitinfos.location = vectoradd(start_location, vectormult(dir, depth));
	return (hitinfos);
}
